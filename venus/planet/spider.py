...
import time, calendar, re, os, urlparse, sys
...
def writeCache(feed_uri, feed_info, data):
    ...
    for updated, entry in ids.values():
        ...
        if not entry.has_key('published_parsed') or not entry['published_parsed']:
        ...
        try:
          elements = minidom.parseString(output).getElementsByTagName("feedworld_mtime")
          if len(elements) > 0:
